...
const PetroleumV2Dashboard: React.FC<PetroleumV2DashboardProps> = ({ stats, startNewInspection, fetchHistory, isLoadingHistory, historyList, onViewReport, onPrint, userRole }) => {
    ...
    return (
      <div className="max-w-6xl mx-auto space-y-6 animate-fadeIn">
          {/* Header Card - Compact */}
          <div className="bg-gradient-to-r from-red-800 to-rose-900 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
               ...
               <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                   <div>
                       <h2 className="text-2xl font-bold mb-4">Petroleum Inspection #2</h2>
                       <div className="flex flex-wrap gap-4">
                           {canCreate && (
                               <button onClick={startNewInspection} className="px-6 py-2.5 bg-white text-rose-900 hover:bg-gray-100 font-bold rounded-xl shadow-lg transition transform hover:scale-105 flex items-center gap-2">
                                   <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"></path></svg>
                                   Start New Inspection
                               </button>
                           )}
                           <button onClick={fetchHistory} className="px-6 py-2.5 bg-black/20 hover:bg-black/30 text-white font-semibold rounded-xl backdrop-blur-sm transition flex items-center gap-2">
                   ...
                   <div className="flex gap-4">
                       <div className="bg-black/20 backdrop-blur-md rounded-xl p-3 text-center min-w-[90px]">
                           ...
                           <div className="text-[10px] text-rose-200 uppercase tracking-wide">Total</div>
                       </div>
                       <div className="bg-black/20 backdrop-blur-md rounded-xl p-3 text-center min-w-[90px]">
                           <div className="text-2xl font-bold">{stats.passRate}%</div>
                           <div className="text-[10px] text-rose-200 uppercase tracking-wide">Compliance</div>
                       </div>
                   </div>
               </div>
          </div>
               ...
               <div className="p-6 border-b border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
                  <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                   ...
                   <div className="relative w-full md:w-72">
                      ...
                      <input type="text" placeholder="Search Truck, Driver or Inspector..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none transition-all" />
                  </div>
              </div>
              {isLoadingHistory ? (
                      ...
                      {displayList.map((item, i) => (
                          <div key={i} className="flex flex-col md:flex-row justify-between items-center p-4 hover:bg-gray-50 transition-colors gap-4">
                              <div className="flex items-center gap-4 w-full md:w-auto">
                                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white ${Number(item.rate) >= 4 ? 'bg-gray-400' : 'bg-gray-500'}`}>
                                      {item.rate}
                                  </div>
                                  <div>
                              ...
                              <div className="flex items-center gap-1 w-full md:w-auto justify-end">
                                  <button onClick={() => onViewReport(item)} title="View & Download Report" className="p-1.5 text-gray-500 bg-white hover:bg-gray-100 border border-gray-200 rounded hover:text-blue-600">
                                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                  </button>
                                  
                                  <button onClick={() => handleShareWhatsApp(item)} title="Share Summary via WhatsApp" className="p-1.5 text-gray-500 bg-white hover:bg-gray-100 border border-gray-200 rounded hover:text-green-600">
                                  ...
                                  <button onClick={() => handleShareEmail(item)} title="Share Summary via Email" className="p-1.5 text-gray-500 bg-white hover:bg-gray-100 border border-gray-200 rounded hover:text-blue-600">
