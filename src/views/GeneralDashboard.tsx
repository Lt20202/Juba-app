...
interface GeneralDashboardProps {
    ...
    userRole?: string; // Passed from App
}

const GeneralDashboard: React.FC<GeneralDashboardProps> = ({ stats, startNewInspection, fetchHistory, isLoadingHistory, historyList, onViewReport, onPrint, userRole }) => {
    ...
    const displayList = useMemo(() => {
        ...
        if (searchTerm.trim()) {
            ...
            // Only show top 10 if not searching
    ...
    return (
      <div className="max-w-6xl mx-auto space-y-6 animate-fadeIn">
          {/* Header Card - Compact */}
          <div className="bg-gradient-to-r from-blue-900 to-slate-800 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
               ...
               <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                   <div>
                       <h2 className="text-2xl font-bold mb-4">General Inspection</h2>
                       <div className="flex flex-wrap gap-4">
                           {canCreate && (
                               <button 
                                 onClick={startNewInspection}
                                 className="px-6 py-2.5 bg-green-500 hover:bg-green-600 text-white font-bold rounded-xl shadow-lg shadow-green-900/20 transition transform hover:scale-105 flex items-center gap-2"
                               >
                           ...
                           <button 
                             ...
                             className="px-6 py-2.5 bg-white/10 hover:bg-white/20 text-white font-semibold rounded-xl backdrop-blur-sm transition flex items-center gap-2"
                           >
                   ...
                   <div className="flex gap-4">
                       <div className="bg-white/10 backdrop-blur-md rounded-xl p-3 text-center min-w-[90px]">
                           <div className="text-2xl font-bold">{stats.total}</div>
                           <div className="text-[10px] text-blue-200 uppercase tracking-wide">Total</div>
                       </div>
                       <div className="bg-white/10 backdrop-blur-md rounded-xl p-3 text-center min-w-[90px]">
                           <div className="text-2xl font-bold">{stats.passRate}%</div>
                           <div className="text-[10px] text-blue-200 uppercase tracking-wide">Safety Score</div>
                       </div>
                   </div>
               </div>
          </div>
